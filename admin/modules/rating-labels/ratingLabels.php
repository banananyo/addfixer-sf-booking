...
class SERVICE_FINDER_ratingLabels extends SERVICE_FINDER_sedateManager{
	...
	public function service_finder_get_labels(){
		...
		$sql = 'SELECT * FROM '.$service_finder_Tables->rating_labels.' WHERE 1=1';

		if( !empty($requestData['search']['value']) ) {
			$sql.=" AND ( label_name LIKE '".$requestData['search']['value']."%' ";    
		...
		foreach($query as $result){
			...
			$nestedData[] = "<input type='checkbox' class='deleteRatingLabelsRow' value='".esc_attr($result->id)."' />";
			$nestedData[] = ucfirst($result->label_name);
			if($result->category_id == 0){
			$nestedData[] = esc_html__('Default', 'service-finder');
			}else{
			$nestedData[] = ucfirst(service_finder_getCategoryName($result->category_id));
			}

...
if($captchaon == 1){
	$wootype = (isset($arg['wootype'])) ? $arg['wootype'] : '';
	if($arg['signup_user_role'] == 'Provider' && $wootype != 'signup'){
...
if(email_exists( esc_attr($arg['signup_user_email']) )){
	...
	if($arg['signup_user_role'] == 'Provider'){
		...
		'slot_interval' => '30',
		'offers_based_on' => 'offers_based_on',
...
global $wpdb, $service_finder_Tables, $service_finder_options;			
			...
			if($loginaftersignup){
			$result = do_action('service_finder_login_after_signup',$_SESSION['signup_username'],$_SESSION['signup_password']);
			if(is_wp_error($result)){
				$pos = strpos($user->get_error_message(), 'Lost your password');
				if (is_int($pos)) {
					$error = explode('<a href',$user->get_error_message());

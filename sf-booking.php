...
if(!class_exists('service_finder_booking_plugin')) {
	class service_finder_booking_plugin {
		...
		public static function service_finder_activate() {
			...
			}						if ( !wp_next_scheduled( 'service_finder_update_mp_payout_status' ) ){				wp_schedule_event( time(), 'daily', 'service_finder_update_mp_payout_status' );				}
		...
		public static function service_finder_deactivate() {
			...
			wp_clear_scheduled_hook('service_finder_update_payout_status');						wp_clear_scheduled_hook('service_finder_update_mp_payout_status');
		...
		**********************/
		...
		public function service_finder_update_mp_payout_status	() {			global $service_finder_options, $wpdb, $service_finder_Tables;							$results = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix.'postmeta'." WHERE `meta_key` = 'mp_payout_due' AND `meta_value` = 'yes'");				if(!empty($results)){					foreach ($results as $row) {					$order_id = $row->post_id;											$bookingid = get_post_meta($order_id,'mp_booking_id',true);					service_finder_get_mp_payout_status($bookingid,$order_id);				}								}		}		
		/**********************
...
var dowlabels = ["'.esc_html__('Mon','service-finder').'", "'.esc_html__('Tue','service-finder').'", "'.esc_html__('Wed','service-finder').'", "'.esc_html__('Thu','service-finder').'", "'.esc_html__('Fri','service-finder').'", "'.esc_html__('Sat','service-finder').'", "'.esc_html__('Sun','service-finder').'"];', 'before' );
		...
		**********************/
		...
		public function service_finder_admin_scripts() {
			...
			if($current_page == 'payout-history'){
		...
		public function service_finder_admin_styles() {
			...
			if( $current_page == 'booking-calendar' || $current_page == 'bookings' || $current_page == 'providers' || $current_page == 'identity-check' || $current_page == 'customers' || $current_page == 'featured-requests' || $current_page == 'payout-history' || $current_page == 'job-category-import' || $current_page == 'wallet-wired-request' || $current_page == 'invoices' || $current_page == 'cities' || $current_page == 'quotations' || $current_page == 'claimbusiness' || $current_page == 'ratinglabels' || $current_page == 'upgraderequest' || $current_page == 'jobconnectrequest' || $current_page == 'providerimport' || $current_page == 'xmlsitemap' || $current_page == 'languages' ){
			/*Bootstrap*/
		...
		$service_finder_options = $service_finder_globals;
			...
			if(is_search() || $author > 0 || is_home() || is_front_page()){
...
function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
	...
	$replacements = array($user->user_login,network_site_url("wp-login.php?action=rp&key=$key&login=" . rawurlencode($user->user_login), 'login'));		

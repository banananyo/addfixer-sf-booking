          ...
          if(!is_user_logged_in()){
            ...
            <div class="tab-content">';
			 ...
			 if($show_signup_otp){ 
			  ...
			  $html .= '<div id="providertab" class="tab-pane fade in active">
					  ...
					  if($allcountry){
						  if(!empty($countries)){
							foreach($countries as $key => $country){
								$html .= '<option value="'.esc_attr($country).'" data-code="'.esc_attr($key).'">'.esc_html__( $country, 'service-finder' ).'</option>';
							}
						  }
					  }else{
					 	 $countryarr = (!empty($service_finder_options['allowed-country'])) ? $service_finder_options['allowed-country'] : '';
						 ...
						 if($countryarr){
						 	foreach($countryarr as $key){
								...
								$html .= '<option '.$select.' value="'.esc_attr($countries[$key]).'" data-code="'.esc_attr($key).'">'. esc_html__( $countries[$key], 'service-finder' ).'</option>';
							}
						 }
					  }
                      $html .= '</select>
...
$label = '<img src="'.SERVICE_FINDER_BOOKING_IMAGE_URL.'/payment/paypal.jpg" title="'.esc_html__('Paypal','service-finder').'" alt="'.esc_html__('Paypal','service-finder').'">';
			 ...
			 $proactive = ($a['role'] == 'customer') ? 'in active' : '';
              $html .= '<div id="customertab" class="tab-pane fade '.$proactive.'">

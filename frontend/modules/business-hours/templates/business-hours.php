...
<div class="panel-heading sf-panel-heading">
  ...
  ?>
  <input <?php echo esc_attr($checked); ?> id="businesshours-active" data-toggle="toggle" data-providerid="<?php echo esc_attr($globalproviderid); ?>" type="checkbox" name="businesshours-active">
  </div>
</div>
...
<input class="bh-onoff" <?php echo esc_attr($checked); ?> data-toggle="toggle" data-weekday="<?php echo esc_attr($key); ?>" type="checkbox" name="<?php echo 'bhstatus_'.esc_attr($key); ?>">
</div>
</div>
<div class="col-md-9">
...
<div class="selected-working-hours clearfix breaktimes-<?php echo esc_attr($key) ?>">
		...
		if(!empty($breaktime[$key])){
			...
			foreach($breaktime[$key] as $breakslot){
				...
				$html .= '<li data-breakslot="'.esc_attr($breakslot).'" data-weekday="'.esc_attr($key).'">'.date('h:i a',strtotime($break[0])).' TO '.date('h:i a',strtotime($break[1])).' <span class="working-hours-remove"><i class="fa fa-close"></i></span></li>';
			}
			$html .= '</ul>';
		}
		echo $html;

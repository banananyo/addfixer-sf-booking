...
';
	...
	public function service_finder_SaveBooking($bookingdata = '',$customerid = '',$txnid = '',$adminfee = 0,$paykey = ''){
		...
		if($payent_mode == 'free' || $payent_mode == 'wired' || $payent_mode == 'cod' || $payent_mode == 'wallet'){
...
Pay Via: %PAYMENTMETHOD%
			...
			$tokens = array('%BOOKINGREFID%','%DATE%','%STARTTIME%','%ENDTIME%','%MEMBERNAME%','%PROVIDERNAME%','%PROVIDEREMAIL%','%PROVIDERPHONE%','%CUSTOMERNAME%','%CUSTOMEREMAIL%','%CUSTOMERPHONE%','%CUSTOMERPHONE2%','%ADDRESS%','%APT%','%CITY%','%STATE%','%ZIPCODE%','%COUNTRY%','%SERVICES%','%PAYMENTMETHOD%','%AMOUNT%','%ADMINFEE%','%SHORTDESCRIPTION%');

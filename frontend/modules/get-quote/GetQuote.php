...
class SERVICE_FINDER_GetQuote{
	...
	public function service_finder_get_quotations($arg){
		...
		foreach($query as $result){
			...
			$nestedData[] = '<button title="'.esc_html__('Edit', 'service-finder').'" data-quoteid="'.esc_attr($result->quoteid).'" data-userid="'.esc_attr($user_id).'" class="btn btn-primary btn-xs quotereply" type="button">'.esc_html__('Edit', 'service-finder').'</button> <button title="'.esc_html__('View', 'service-finder').'" data-quoteid="'.esc_attr($result->quoteid).'" class="btn btn-primary btn-xs viewquotation" type="button">'.esc_html__('View', 'service-finder').'</button>';
			}else{
			$nestedData[] = '<button title="'.esc_html__('Reply', 'service-finder').'" data-quoteid="'.esc_attr($result->quoteid).'" data-userid="'.esc_attr($user_id).'" class="btn btn-primary btn-xs quotereply" type="button">'.esc_html__('Reply', 'service-finder').'</button> <button title="'.esc_html__('View', 'service-finder').'" data-quoteid="'.esc_attr($result->quoteid).'" class="btn btn-primary btn-xs viewquotation" type="button">'.esc_html__('View', 'service-finder').'</button>';
			}
	...
	public function service_finder_get_customer_quotations($arg){
		...
		foreach($query as $result){
			...
			$nestedData[] = '<button title="'.esc_html__('Number of Replies', 'service-finder').'" data-quoteid="'.esc_attr($result->id).'" data-userid="'.esc_attr($user_id).'" class="btn btn-primary btn-xs showreplies" type="button">'.esc_html__('View Replies', 'service-finder').' <span class="sf-quote-reply">( '.$totalreplies.' )</span>'.'</button> <button title="'.esc_html__('View', 'service-finder').'" data-quoteid="'.esc_attr($result->id).'" class="btn btn-primary btn-xs viewquotation" type="button">'.esc_html__('View', 'service-finder').'</button>';
...
';
			...
			if($service_finder_options['request-quote-mail-to'] == 'provider'){
				...
				if(service_finder_wpmailer($adminemail,$adminmsg_subject,$adminmsg_body) && service_finder_wpmailer($provideremails,$msg_subject,$msg_body)) {
					$success = array(
							'status' => 'success',
							'suc_message' => $msg
							);

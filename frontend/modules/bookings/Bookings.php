...
Services: %SERVICES%';
	...
	public function service_finder_getBookings($arg){
		...
		$members = $wpdb->get_results($wpdb->prepare('SELECT bookings.id, bookings.multi_date, bookings.payment_to, bookings.charge_admin_fee_from, bookings.paid_to_provider, bookings.total, bookings.adminfee, bookings.payment_type, bookings.type, bookings.jobid, bookings.quoteid, bookings.wired_invoiceid, bookings.date, bookings.start_time, bookings.end_time, members.member_name, bookings.member_id, bookings.status, bookings.txnid, customers.name, customers.phone, customers.email, customers.address, customers.city FROM '.$service_finder_Tables->bookings.' as bookings INNER JOIN '.$service_finder_Tables->customers.' as customers on bookings.booking_customer_id = customers.id LEFT JOIN '.$service_finder_Tables->team_members.' as members on bookings.member_id = members.id WHERE `provider_id` = %d',$user_id));
		...
		$sql = $wpdb->prepare("SELECT bookings.id, bookings.jobid, bookings.quoteid, bookings.multi_date, bookings.payment_type, bookings.payment_to, bookings.charge_admin_fee_from, bookings.paid_to_provider, bookings.total, bookings.adminfee, bookings.type, bookings.wired_invoiceid, bookings.date, bookings.start_time, bookings.end_time, members.member_name, bookings.member_id, bookings.status, bookings.txnid, customers.name, customers.phone, customers.email, customers.address, customers.city FROM ".$service_finder_Tables->bookings." as bookings INNER JOIN ".$service_finder_Tables->customers." as customers on bookings.booking_customer_id = customers.id LEFT JOIN ".$service_finder_Tables->team_members." as members on bookings.member_id = members.id WHERE `provider_id` = %d",$user_id);
		$query=$wpdb->get_results($sql);
		$totalData = count($query);
		$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.
		
		
		$sql = "SELECT bookings.id, bookings.jobid, bookings.quoteid, bookings.payment_to, bookings.multi_date, bookings.payment_type, bookings.charge_admin_fee_from, bookings.paid_to_provider, bookings.total, bookings.adminfee, bookings.type, bookings.wired_invoiceid, bookings.date, bookings.start_time, bookings.end_time, members.member_name, bookings.member_id, bookings.status, bookings.txnid, customers.name, customers.phone, customers.email, customers.address, customers.city";
...
';
			...
			$bookingdetails = '<div class="sf-booking-info-col">
							...
							$bookingdetails .= '<li><strong><i class="fa fa-calendar-o"></i> '.esc_html__('Date', 'service-finder').'</strong> '.$result->date.'</li>
			...
			$paymentdetails = '<div class="inner">
										<h3><span class="sf-booking-payment-info" data-toggle="popover" data-container="body" data-placement="top" type="button" data-html="true" id="payinfo-'.$result->id.'" data-trigger="hover">'.service_finder_money_format($result->total).' </span><span class="sf-payment-status">'.$paymentstatus.'</span></h3>
...
';
		...
		if($row->status != "Cancel" && $row->status != "Completed" && $isadmin != "yes"){
			if($row->paid_to_provider == 'pending' && $row->paypal_paykey != "" && $payment_methods['paypal-adaptive']){
			$complete .= '<button type="button" data-paykey="'.esc_attr($row->paypal_paykey).'" data-providerid="'.esc_attr($row->provider_id).'" class="btn btn-primary completebookingandpay" data-id="'.esc_attr($bookingid).'">'.esc_html__('Mark As Complete', 'service-finder').'</button>';
			$payadaptiveflag = 1;
  ...
  <select title="'.esc_html__('Option', 'service-finder').'" class="bookingOptionSelect form-control sf-form-control sf-select-box" data-bid="'.esc_attr($result->id).'">
	...
	public function service_finder_getBookedServices($arg){
		...
		foreach($query as $result){
			...
			$nestedData[] = date('d-m-Y',strtotime($result->date));
	...
	public function service_finder_getCustomerUpcomingBookings(){
		...
		// getting total number records without any search
		$sql = $wpdb->prepare("SELECT bookings.*, providers.full_name, providers.avatar_id, providers.phone, providers.email, customers.address FROM ".$service_finder_Tables->bookings." as bookings INNER JOIN ".$service_finder_Tables->customers." as customers INNER JOIN ".$service_finder_Tables->providers." as providers on bookings.booking_customer_id = customers.id AND bookings.provider_id = providers.wp_user_id WHERE ((bookings.date >= CURDATE() AND bookings.multi_date != 'yes') OR (bookings.status = 'Pending' AND bookings.multi_date = 'yes')) AND customers.`wp_user_id` = %d",$currUser->ID);
		$query=$wpdb->get_results($sql);
		$totalData = count($query);
		$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.
		...
		$sql.=" FROM ".$service_finder_Tables->bookings." as bookings INNER JOIN ".$service_finder_Tables->customers." as customers INNER JOIN ".$service_finder_Tables->providers." as providers on bookings.booking_customer_id = customers.id AND bookings.provider_id = providers.wp_user_id WHERE ((bookings.date >= CURDATE() AND bookings.multi_date != 'yes') OR (bookings.status = 'Pending' AND bookings.multi_date = 'yes')) AND customers.`wp_user_id` = ".$currUser->ID;
		if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
			$sql.=" AND ( customers.name LIKE '".$requestData['search']['value']."%' ";    
			$sql.=" OR bookings.id LIKE '".$requestData['search']['value']."%' ";
			$sql.=" OR bookings.date LIKE '".$requestData['search']['value']."%' ";
...
Pay Via: %PAYMENTMETHOD%
			...
			$replacements = array($bookingid,date('Y-m-d',strtotime($bookingInfo->date)),$bookingInfo->start_time,$bookingInfo->end_time,$membername,service_finder_get_providername_with_link($providerInfo->wp_user_id),$providerInfo->email,service_finder_get_contact_info($providerInfo->phone,$providerInfo->mobile),$customerInfo->name,$customerInfo->email,$customerInfo->phone,$customerInfo->phone2,$customerInfo->address,$customerInfo->apt,$customerInfo->city,$customerInfo->state,$customerInfo->zipcode,$customerInfo->country,ucfirst($bookingInfo->type),service_finder_money_format($bookingInfo->total));
			$msg_body = str_replace($tokens,$replacements,$message);
			$msg_subject = 'Booking Completed';
